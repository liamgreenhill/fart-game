...
function gameRules(){
  ...
  const isValidInput = validateInputWord(seedWord, inputWord);

  // If the input word is valid, display it on the screen
  if (isValidInput) {
    ...
    // Update the seed word to be the previous guess
    seedWord = inputWord;

    // Clear the input field
    inputElement.value = "";

    // Focus on the input field, ready for the next guess
...
function generateRandomWord() {
  ...
  const randomNumber = Math.floor(Math.random() * eligibleWords.length);

  // Return the word at the random index
  return eligibleWords[randomNumber];
}

// Function to validate the input word
function validateInputWord(seedWord, inputWord) {
  // Check if the input word is four letters long
  if (inputWord.length !== 4) {
    errorCode = "Wrong length. It's got to be a four letter word."
    console.log(errorCode);
    return false;
  }
  ...
  // Check if three of the letters in the input word are the same as the seed word and appear in the same position
  let matchingLetters = 0;
  for (let i = 0; i < seedWord.length; i++) {
    if (seedWord[i] === inputWord[i]) {
      matchingLetters++;
    }
  }
  ...
  // All checks passed, the input word is valid
  return true;
}